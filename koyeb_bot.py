...
🌐 Deploy on Koyeb for 24/7 FREE hosting
...
def start_keep_alive_server():
    """Start HTTP server for health checks (Koyeb requirement)"""
    try:
        ...
        logger.error(f"❌ Could not start keep-alive server: {e}")
        return False
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                if format_id:
                    ...
                    return {
                        ...
                        'url': format_info['url'] if 'url' in format_info else url,
...
📊 *Your Stats:*
{await get_user_stats_text(user.id)}
...
👤 *User:* {update.effective_user.first_name if 'update' in locals() else 'User'}
🆔 *ID:* `{user_id}`
...
async def calculate_user_rank(user_id: int) -> int:
    """Calculate user rank based on total downloads"""
    try:
        all_users = db.get_all_users()
        sorted_users = sorted(all_users, key=lambda x: x[3] if x[3] else 0, reverse=True)
...
async def button_callback(update: Update, context: CallbackContext):
    ...
    if data.startswith("guide_"):
        ...
        # Store rating (you can save to database)
        await query.message.edit_text(
        ...
        # Handle admin user list pagination
        pass
...
async def show_leaderboard(update: Update, context: CallbackContext):
    ...
    current_rank = await calculate_user_rank(current_user_id)
    
    leaderboard_text += f"📊 *Your rank:* #{current_rank}"
...
async def main():
    ...
    print("🌐 Optimized for Koyeb 24/7 Hosting")
    print("=" * 60)
    
    # Start keep-alive server for Koyeb
